...
namespace Covid_19_Statistics.Services
{
    ...
    {
        public static string baseUri = "ms-appx:///Assets/Maps/";
        ...
        public static async Task<MapModel> GetMapBySVGAsync(String countryId, String name)
        {
            MapModel mapModel = new MapModel() { Name = name };
            var file = await StorageFile.GetFileFromApplicationUriAsync(new Uri(baseUri + "/" + countryId + "/" + name + ".svg"));
            var doc = await XmlDocument.LoadFromFileAsync(file);
